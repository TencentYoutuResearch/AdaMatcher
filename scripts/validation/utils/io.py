...
def parse_image_list(path: Path,
    ...
    with open(path, 'r') as f:
        for line in f:
            ...
            if with_intrinsics:
                ...
                camera = Camera(None, camera_model, int(width), int(height),
                                params)
...
def write_pose_results(pose_dict: Dict,
                       outfile: Path,
...
def concat_results(paths: List[Path], names: List[Union[int, str]],
    ...
    output_path = str(output_path).replace(f'{{{key}}}',
                                           '-'.join(str(n)[:3] for n in names))
