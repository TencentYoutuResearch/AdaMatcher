            ...
            if val is not None:
                if attr in {'dataset', 'dumps'}:
                    paths[attr] = val
                        ...
                        'query_images', 'reference_images', 'ground_truth'
                }:
...
def create_argparser(dataset: str) -> argparse.ArgumentParser:
    ...
    parser.add_argument('--eval_dir', type=Path, default=settings.EVAL_PATH)
...
def parse_paths(args, default_paths: Paths) -> Paths:
    default_paths = default_paths.add_prefixes(args.dataset, args.dumps,
                                               args.eval_dir)
    paths = {}
