...
class RandomConcatSampler(Sampler):
    ...
    `subset_replacement` is ``True``, sampling within each subset will be done
    with replacement. However, it is impossible to sample data without
    replacement between epochs, unless building a stateful sampler lived along
    ...
    NOTE: This sampler behaves differently with DistributedSampler.
          It assume the dataset is split across ranks instead of replicated.
    TODO: Add a `set_epoch()` method to fulfill sampling without replacement across epochs.
